...
# ⚠️ The suffix is the OBJECTIVE ID, which the platform derives from `gating`
# in box.yaml — uppercased, with every non-alphanumeric turned into `_`. This
# host's flags are gated `service-user` and `root`, so they arrive as
# DESTRIER_FLAG_SERVICE_USER and DESTRIER_FLAG_ROOT. Reading the wrong name is
# silent: the fallback below plants a fixed value, the run scores zero on that
# objective, and nothing anywhere says why.
USER_FLAG="${DESTRIER_FLAG_SERVICE_USER:-destrier{local-dev-user}}"
